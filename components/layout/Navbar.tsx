      ...
      {isOpen ? (
        <div className="border-t border-slate-200 bg-white px-4 py-4 md:hidden">
          <div className="flex flex-col gap-4">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-sm font-medium text-slate-700" onClick={() => setIsOpen(false)}>
                {item.label}
              </Link>
            ))}
            <Link href="/dashboard" className="text-sm font-medium text-slate-700" onClick={() => setIsOpen(false)}>
              Dashboard
            </Link>
            {session?.user ? (
              <button
              ...
              <Link href="/auth" className="text-sm font-medium text-slate-700" onClick={() => setIsOpen(false)}>
                Login
              </Link>
            )}
